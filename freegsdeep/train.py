...
from freegsdeep.model import PINTO, DeepONet_resi
...
class Trainer_resi():
    ...
    def loss(
        ...
        y = self.model.forward(
            R.unsqueeze(0).unsqueeze(2), Z.unsqueeze(0).unsqueeze(2), rhs
            )
        ...
        output_branch = self.model.branch_source(rhs).reshape(rhs.shape[0], -1)
        ...
        for epoch in range(load_epoch, self.epoch):
            ...
            for i, batch in enumerate(train_dataloader):
                ...
                if epoch < 5 and (i + 1) % 50 == 0:
                    ...
                    self.image(
                        i, sample_rhs[0], bdry[0], predict,
                        soln[0], coil[0], save_path=save_path
                        )
                    print(
            ...
            if (epoch + 1) % 1 == 0:
                ...
                predict = sample_forward * (sample_rhs.max() - sample_rhs.min())
                self.image(
                    epoch, sample_rhs[0], bdry[0], predict,
                    soln[0], coil[0], save_path=save_path
                    )
...
class Trainer_bdry():
    ...
    def train(self, save_name: str, load_name: Optional[dict] = None) -> None:    
        ...
        self.resi_pt = torch.rand(
            (1, self.num_bdry_pt, 2), dtype=torch.float64, device=self.device
        )
