...
class constrain(object):
    ...
    def __init__(
        self,
        xpoints: Optional[Tensor] = None,
        gamma: float = 1e-12,
        isoflux: Optional[Tensor] = None,
        psivals: Optional[Tensor] = None,
        current_lims: Optional[List[Tuple[float, float]]] = None,
    ...
    def __call__(self, eq, synthetic: bool = False):
        ...
        if self.xpoints is not None:
            psi, dpsi_dR, dpsi_dZ, _, _, _ = \
                eq.critical.compute_grad(
                    self.xpoints[:, 0], self.xpoints[:, 1], eq.rhs,
                    eq.bdry, eq.scaling_rhs, eq.scaling_bdry, synthetic
                    )
            Br = -dpsi_dZ / self.xpoints[:, 0] + eq.tokamak.Br(
                self.xpoints[:, 0], self.xpoints[:, 1]
                )
            constraint_rhs.append(-Br)
            constraint_matrix.append(
                tokamak.controlBr(self.xpoints[:, 0], self.xpoints[:, 1]).T
                )
            Bz = dpsi_dR / self.xpoints[:, 0] + eq.tokamak.Bz(
                self.xpoints[:, 0], self.xpoints[:, 1]
                )
            constraint_rhs.append(-Bz)
            constraint_matrix.append(
                tokamak.controlBz(self.xpoints[:, 0], self.xpoints[:, 1]).T
                )
        if self.isoflux is not None:
            p1, _, _, _, _, _ = \
                eq.critical.compute_grad(
                    self.isoflux[:, 0], self.isoflux[:, 1], eq.rhs,
                    eq.bdry, eq.scaling_rhs, eq.scaling_bdry, synthetic
                    )
            p1 += eq.tokamak.psi(
                self.isoflux[:, 0], self.isoflux[:, 1]
                )
            p2, _, _, _, _, _ = \
                eq.critical.compute_grad(
                    self.isoflux[:, 2], self.isoflux[:, 3], eq.rhs,
                    eq.bdry, eq.scaling_rhs, eq.scaling_bdry, synthetic
                    )
            p2 += eq.tokamak.psi(
                self.isoflux[:, 2], self.isoflux[:, 3]
                )
            constraint_rhs.append(p2 - p1)
            c1 = tokamak.controlPsi(self.isoflux[:, 0], self.isoflux[:, 1])
            c2 = tokamak.controlPsi(self.isoflux[:, 2], self.isoflux[:, 3])
            c = torch.stack([c1val - c2val for c1val, c2val in zip(c1, c2)]).T
            constraint_matrix.append(c)
        if self.psivals is not None:
            p1, _, _, _, _, _ = \
                eq.critical.compute_grad(
                    self.psivals[:, 0], self.psivals[:, 1], eq.rhs,
                    eq.bdry, eq.scaling_rhs, eq.scaling_bdry
                    )
            p1 += eq.tokamak.psi(
                self.psivals[:, 0], self.psivals[:, 1]
                )
            constraint_rhs.append(self.psivals[:, 2] - p1)
            c = tokamak.controlPsi(self.psivals[:, 0], self.psivals[:, 1])
            constraint_matrix.append(c)
        ...
        # Constraint matrix
        A = torch.concatenate(constraint_matrix, dim=0)
        b = torch.concatenate(constraint_rhs).reshape(-1)
        ...
        self.current_change = torch.linalg.inv(
            A.T @ A + self.gamma ** 2 * torch.eye(
                ncontrols, dtype=torch.float64, device=A.device
                )
            ) @ A.T @ b
        ...
        if self.current_lims is None:
            for i in range(ncontrols):
                current_change_bounds.append((-np.inf, np.inf))
        else:
        ...
        current_change_bnds = np.array(current_change_bounds)

        # Reform the constraint matrices to include Tikhonov regularisation
        A2 = np.concatenate([A.detach().cpu().numpy(), self.gamma * np.eye(ncontrols)])
        b2 = np.concatenate([b.detach().cpu().numpy(), np.zeros(ncontrols)])
        ...
        if self.current_change.shape[0] > 0:
            x0 = self.current_change.detach().cpu().numpy()
            sol = optimize.minimize(
                objective, x0, method="SLSQP", bounds=current_change_bnds, constraints=cons
            )

            self.current_change = torch.tensor(sol.x, dtype=torch.float64, device=A.device)
