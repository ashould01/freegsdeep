...
class ConstrainPaxisIp(Profile):
    ...
    def Jtor(
        ...
        self.eq._updateBoundaryPsi(psi, synthetic=True)
        psi_bndry = self.eq.psi_bndry # Here we debug
        print(psi_bndry)

        # Analyse the equilibrium, finding O- and X-points
        opt, xpt = self.eq.critical.find_critical(
            self.eq.R, self.eq.Z, 
            self.eq.rhs.reshape(1, 1, self.eq.nx, self.eq.ny), self.eq.bdry, 
            psi, self.eq.tokamak, synthetic=True 
            )
        if not opt:
