...
class critical():
    ...
    def find_critical(
        self, R: Tensor, Z: Tensor, psi: Tensor, discard_xpoints=True
        ):
        """
                    ...
                    while True:
                        ...
                        if Br ** 2 + Bz ** 2 < 1e-6:
                            ...
                            if D < 0.0:
                                ...
                                xpoint.append((R1, Z1, f(R1, Z1)))
                            else:
                                # Found O-point
                                # opoint.append((R1, Z1, f(R1, Z1)[0][0]))
                                opoint.append((R1, Z1, f(R1, Z1)))
                            break
                        ...
                        J[0, 0] = -Br / R1 - f(R1, Z1, dy=1, dx=1) / R1
                        J[0, 1] = -f(R1, Z1, dy=2) / R1
                        J[1, 0] = -Bz / R1 + f(R1, Z1, dx=2) / R1
                        J[1, 1] = f(R1, Z1, dx=1, dy=1) / R1

                        d = torch.matmul(
                            torch.linalg.inv(J),
                            torch.tensor([Br, Bz], dtype=torch.float64, device=self.device)
                            )
        ...
        if discard_xpoints:
            ...
            for xpt in xpoint:
                ...
                rline = torch.linspace(Ro, Rx, steps=50, dtype=torch.float64, device=self.device)
                zline = torch.linspace(Zo, Zx, steps=50, dtype=torch.float64, device=self.device)
    ...
    def core_mask(self, R, Z, psi, opoint, xpoint=[], psi_bndry=None):
        """
        ...
        """
        mask = torch.zeros(psi.shape, device=self.device)
        nx, ny = psi.shape
        ...
        for rx, zx, _ in xpoint:
            # Find nearest index
            ix = torch.argmin(torch.abs(R[:, 0] - rx)).item()
            jx = torch.argmin(torch.abs(Z[0, :] - zx)).item()
        ...
        rind = torch.argmin(torch.abs(R[:, 0] - Ro)).item()
        zind = torch.argmin(torch.abs(Z[0, :] - Zo)).item()
    ...
    def find_psisurface(self, eq, psifunc, r0, z0, r1, z1, psival=1.0, n=100, axis=None):
        """
        ...
        if abs(r1 - r0) > 1e-6:
            rclip = torch.clip(r1, eq.Rmin, eq.Rmax)
            z1 = z0 + (z1 - z0) * abs((rclip - r0) / (r1 - r0))
            r1 = rclip

        if abs(z1 - z0) > 1e-6:
            zclip = torch.clip(z1, eq.Zmin, eq.Zmax)
            r1 = r0 + (r1 - r0) * abs((zclip - z0) / (z1 - z0))
            z1 = zclip

        r = torch.linspace(r0, r1, n, device=self.device)
        z = torch.linspace(z0, z1, n)
    ...
    def find_separatrix(
        self, eq, opoint=None, xpoint=None, ntheta=20, psi=None, axis=None, psival=1.0
    ):
        ...
        if (opoint is None) or (xpoint is None):
            opoint, xpoint = self.find_critical(eq.R, eq.Z, psi)

        psinorm = (psi - opoint[0][2]) / (eq.psi_bndry - opoint[0][2])

        psifunc = TorchBicubicSpline2D(eq.R[:, 0], eq.Z[0, :], psinorm)

        r0, z0 = opoint[0][0:2]

        theta_grid = torch.linspace(
            0, 2 * torch.pi, ntheta, endpoint=False,
            dtype=torch.float64, device=self.device
            )
        dtheta = theta_grid[1] - theta_grid[0]

        # Avoid putting theta grid points exactly on the X-points
        xpoint_theta = torch.arctan2(xpoint[0][0] - r0, xpoint[0][1] - z0)
        ...
        for theta in theta_grid:
            r, z = self.find_psisurface(
                eq,
                psifunc,
                r0,
                z0,
                r0 + 10.0 * torch.sin(theta),
                z0 + 10.0 * torch.cos(theta),
                psival=psival,
                axis=axis,
                n=1000,
            )
    ...
    def find_safety(
        self, eq, npsi=1, psinorm=None, ntheta=128, psi=None,
        opoint=None, xpoint=None, axis=None
    ):
        ...
        psifunc = TorchBicubicSpline2D(eq.R[:, 0], eq.Z[0, :], psinormal)
        ...
        if psinorm is None:
            npsi = 100
            psirange = torch.linspace(
                1.0 / (npsi + 1), 1.0, npsi, endpoint=False,
                dtype=torch.float64, device=self.device
                )
        else:
            try:
        ...
        psisurf = torch.zeros([npsi, ntheta, 2], dtype=torch.float64, device=self.device)
        ...
        for i in range(npsi):
            ...
            for j in range(ntheta):
                ...
                r, z = self.find_psisurface(
                    ...
                    r0,
                    z0,
                    r0 + torch.ptp(eq.R) * torch.sin(theta),
                    z0 + torch.ptp(eq.Z) * torch.cos(theta),
                    psival=psin,
                    axis=axis,
                )
                psisurf[i, j, :] = [r, z]
        ...
        Br = eq.Br(r, z)
        Bz = eq.Bz(r, z)
        Bthe = torch.sqrt(Br ** 2 + Bz ** 2)

        # Differentiate location w.r.t. index
        dr_di = (torch.roll(r, 1, axis=1) - torch.roll(r, -1, axis=1)) / 2.0
        dz_di = (torch.roll(z, 1, axis=1) - torch.roll(z, -1, axis=1)) / 2.0

        # Distance between points
        dl = torch.sqrt(dr_di ** 2 + dz_di ** 2)

        # Integrand - Btor/(R*Bthe) = Fpol/(R**2*Bthe)
        qint = fpol / (r ** 2 * Bthe)

        # Integral
        q = sum(qint * dl, axis=1) / (2 * torch.pi)
