...
from freegsdeep.utils.jtor import ConstrainPaxisIp as ConstrainPaxisIp_jax
...
class main():
    ...
    def __init__(
        ...
        with open(
            'logs/260616_integratednet/model/model_40.eqx', 'rb'
            ) as f:
    ...
    def simulation(
        ...
        while (control_trial_psi is False) and (n_up < 10):
            try:
                res0 = self.F_function(
                    jnp.asarray(trial_plasma_psi), self.solver.tokamak_psi, profiles
                    )
        ...
        while (rel_change > target_relative_tolerance) * (
            ...
            psi = trial_plasma_psi.reshape(self.nx, self.ny) + \
                self.solver.tokamak_psi.reshape(self.nx, self.ny)
            levels = np.linspace(psi.min(), psi.max(), 50)
            c0 = ax[0].contour(eq.R, eq.Z, psi, levels=levels)
            ...
            c2 = ax[2].imshow(update.reshape(self.nx, self.ny).T, origin='lower')
            fig.colorbar(c2, ax=ax[2])
            eq.plot(axis=ax[1], show=False)
            save_path = os.path.join('image', 'MAST-U_2', image_path)
            os.makedirs(save_path, exist_ok=True)
            fig.savefig(save_path + '/iteration_' + str(iterations) + '.png')
            plt.close()
            print(profiles.psi_bndry)
            np.savez(
                save_path + '/iteration_' + str(iterations) + '.npz',
                trial_plasma_psi=np.asarray(trial_plasma_psi),
                psi=psi,
...
if __name__ == "__main__":
    sim = main(
        ...
        nR=65, nZ=129, params=(1e4, 5e5, 0.5)
    )
    # import cProfile
    # import pstats
    # import io
    ...
    sim.simulation(
        image_path='simulation_bdry_x_out_o_in_o', picard_handover=0.1,
        target_relative_tolerance=1e-18
        )
